...
-- building avarage
SELECT f.building_code, b.building_name, COUNT(*) AS 'quantity', AVG(dbm.percent) AS 'avarage percent'
FROM form AS f, building AS b, ssid_dbm AS dbm
WHERE b.building_code='RS-OF' AND f.building_code = b.building_code AND f.form_id = dbm.form_id
GROUP BY f.building_code;

-- form avreage
SELECT f.form_id, f.timestamp, f.building_code, b.building_name, f.floor, 
f.detail, COUNT(*) AS 'quantity', AVG(dbm.percent) AS 'average_percent'
FROM form AS f, building AS b, ssid_dbm AS dbm
WHERE b.building_code = 'RS-OF' AND f.building_code = b.building_code AND f.form_id = dbm.form_id
GROUP BY f.form_id;
